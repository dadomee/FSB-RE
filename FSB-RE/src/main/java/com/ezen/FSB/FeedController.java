...
public class FeedController {
	...
	public ModelAndView profileUpdateOK(@RequestParam Map<String, Object> map, HttpServletRequest req) {
		...
		System.out.println("멤버번호:"+mem_num+" 닉네임:"+mem_nickname+" 상메"+prof_msg);
		System.out.println("닉넴분리:"+old_prof_nickname_separated);
		...
		System.out.println((String)map.get("prof_open"));
		System.out.println("잠금:" + prof_open);
		...
		if(isDef==1) { //(디폴트) 디폴트 체크
			...
			System.out.println("이미지를 디폴트로 설정");
	...
	public ModelAndView themeTimeline(HttpServletRequest req, int theme_num) {
		...
		System.out.println(theme_num+"테마번호, 멤버번호"+mem_num);
		ModelAndView mav = new ModelAndView("feed/timeLine");
		mav.addObject("listFeed", list);
